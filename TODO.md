...
- [ ] 已经有单独的标签管理界面了，'全部'界面的标签管理可以去掉
- [ ] 标签管理界面添加、编辑标签的弹窗与主题风格不搭，且编辑标签、删除标签的操作过于复杂，需要简化操作
- [ ] 标签的设置过于复杂，可以通过下拉菜单(多选)的方式进行设置
- [ ] 导入大文件夹(有多层子文件夹，对应多个合集)，软件会出现未响应的情况，可以优化一下性能或者限制一下速度，可以慢，但不能未响应
- [ ] 导入的进度显示可以优化一下，当批量导入(含多个子文件的大文件夹时)，可以显示总的合集个数以及当前已完成的个数，有进度条显示，更为直观
- [ ] 查看集合图片的时候，当图片的高度过长时，会出现图片越界超过容器的情况，需要优化
- [ ] 查看集合里的图片时可以选择网格视图，当使用网格视图时，只展示缩略图和名字，方便浏览集合图片
